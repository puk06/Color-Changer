...
internal partial class HelpForm : Form
{
    ...
    private static readonly string SelectionPenToolMessage = "マウスペンで選択エリアを追加 / 削除する。\n" +
        "- テクスチャ上の画像を、マウスでなぞるようにして選択・削除ができます。\n\n" +
        "1. 選択モードを有効にします。\n" +
        "2. 画面上部の「ツール」メニューから「選択用ペンツール」をクリックします。\n" +
        "3. 表示されたウィンドウでペンを有効化し、スライダーで太さを調整します。\n" +
        "4. メイン画面または拡大プレビュー上の画像を、マウスでドラッグしてなぞると選択できます。\n" +
        "5. 「選択用レイヤー」「消去用レイヤー」「取り消し」のいずれかを選択して、操作内容を決定してください。\n\n" +
        "　- 選択用レイヤー: 通常通り、オブジェクトを選択するためのレイヤーです。\n" +
